...
const LNR_JETTON_MASTER = "EQArZiLoiwGQVFXf-xypL_3qBg6DYuOQTHNJhOmEju-LNPuk";

async function holdingWalletBalance(env, wallet) {
  // Read the real LNR Jetton balance from the connected TON wallet.
  // The LNR master is fixed here so a missing Cloudflare variable cannot
  // silently make the Holding Wallet show zero.
  if (!safeWallet(wallet)) return 0;
  ...
  try {
    ...
    const target = String(
      env.LNR_JETTON_MASTER || LNR_JETTON_MASTER
    ).trim().toLowerCase();

    const balances = Array.isArray(data?.balances) ? data.balances : [];
    const item = balances.find(x => {
      const jettonAddress = String(
        x?.jetton?.address ||
        x?.jetton?.master ||
        x?.jetton?.jetton_address ||
        ""
      ).trim().toLowerCase();
      return jettonAddress === target;
    });

    if (!item) return 0;

    const decimals = num(item?.jetton?.decimals, 9);
    const rawBalance = num(item?.balance, 0);
