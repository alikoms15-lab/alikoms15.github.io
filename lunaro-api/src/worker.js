...
const TREASURY_OWNER = "UQCNobW-3Kzoku_up0oV0yD3pUegqj0q3S2ptCfp2yhKilD7";
...
async function processWithdrawal(env, withdrawal) {
  ...
  const payout=Number(withdrawal.payout_amount ?? withdrawal.net_amount);
  const requestedAmount=Number(withdrawal.requested_amount ?? withdrawal.amount);
  ...
  try{
    ...
    await env.DB.prepare(`UPDATE withdrawals SET status='completed',tx_hash=?,completed_at=CURRENT_TIMESTAMP WHERE id=? AND status='processing'`).bind(txHash,id).run();
    return {success:true,withdrawal_id:id,telegram_id:telegramId,payout,tx_hash:txHash};
  }catch(error){
    const failed=await env.DB.prepare(`UPDATE withdrawals SET status='failed',completed_at=CURRENT_TIMESTAMP WHERE id=? AND status='processing'`).bind(id).run();
...
async function processPendingWithdrawals(env){
  const rows=await env.DB.prepare(`SELECT id,telegram_id,wallet,requested_amount,payout_amount,status,tx_hash,created_at FROM withdrawals WHERE status='pending' ORDER BY id ASC LIMIT 5`).all();
