...
async function holdingWalletBalance(env, wallet) {
  // Returns:
  //   { ok: true, balance: <real LNR balance> } when blockchain answered.
  //   { ok: false, balance: 0 } when the request failed/was unavailable.
  // This distinction is important: a temporary RPC/API failure must NEVER
  // turn a previously valid Holding Wallet balance into zero.
  ...
  try {
    ...
    if (!r.ok) return { ok: false, balance: 0 };
    ...
    // No row with exclude_zero_balance=true is a valid blockchain answer:
    // the wallet currently has no LNR balance.
    if (!item) return { ok: true, balance: 0 };

    // LNR uses 6 decimals. The raw on-chain value is divided by 1e6.
...
async function buildUserPayload(env, user) {
  ...
  // Keep the last known valid value as the display fallback.
  let holding = num(user?.holding_wallet, 0);

  const chainResult = await holdingWalletBalance(env, wallet);

  // Only replace the previous value when the blockchain response itself
  // was valid. A temporary timeout/error leaves the previous value intact.
  if (chainResult.ok) {
