...
const { Address, beginCell, SendMode, internal } = await import("@ton/core");
...
async function sendLnr(
  ...
  /*
   * Build the standard TEP-74 Jetton transfer body directly.
   *
   * The @ton/ton JettonWallet wrapper used by the previous build does
   * not expose sendTransfer() in this package/version. Calling that
   * missing helper caused: "jettonWallet.sendTransfer is not a function".
   *
   * We therefore send the transfer through the treasury's TON wallet
   * contract directly, targeting the treasury Jetton wallet address.
   */
  const transferBody =
    beginCell()
      ...
      .storeAddress(treasury.wallet.address)
      .storeBit(0)
      .storeCoins(1000000n)
      .storeBit(0)
      .endCell();
...
async function bestEffortSchema(env) {
  ...
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS withdrawal_processor_lock (
...
async function handleTask(
  ...
  try {
    ...
    /* FAST PATH: process immediately; Cron remains the retry/fallback. */
    if (ctx && typeof ctx.waitUntil === "function") {
      ctx.waitUntil(
        processPendingWithdrawals(env).catch((error) => {
          console.error("Immediate withdrawal processing error:", error);
        })
      );
    }
...
async function handleWithdraw(
  ...
  /* FAST WITHDRAW PATH:
     Start processing immediately after the withdrawal row is committed.
     Cloudflare keeps this work alive via waitUntil while the response
     returns to the user. Cron remains only as a retry/fallback path. */
  if (ctx && typeof ctx.waitUntil === "function") {
    ctx.waitUntil(
      processPendingWithdrawals(env).catch((error) => {
        console.error("Immediate withdrawal processing error:", error);
...
async function keepWithdrawalPendingAfterRateLimit(env, id, stage, error) {
  await saveWithdrawalError(env, id, stage, error, {
    rate_limited: true,
    action: "kept_pending_for_retry",
    retry_policy: "next_cron_run"
  });
  ...
  return {
    ...
    withdrawal_id: id,
    error_stage: stage,
    error_message: String(error?.message || error || "Rate limited by TON API"),
    refunded: false,
    retry: true
  };
}
...
async function processWithdrawal(
  ...
  try {
    ...
    if (isRateLimitedError(error)) {
      return await keepWithdrawalPendingAfterRateLimit(
        env,
        id,
        "treasury",
        error
  ...
  /*
   * Stage 4:
   * Wait briefly for the treasury seqno to advance.
   * Broadcast itself happens in sendLnr; this wait is only for
   * confirming the wallet accepted the external message.
   */
...
/* =========================================================
   WITHDRAWAL PROCESSOR LEASE
   ========================================================= */

async function acquireWithdrawalProcessorLock(env) {
  try {
    await env.DB.prepare(`
      INSERT OR IGNORE INTO withdrawal_processor_lock
      (id, locked_until)
      VALUES (1, 0)
    `).run();

    const now = Date.now();
    const leaseUntil = now + 240000;

    const result = await env.DB.prepare(`
      UPDATE withdrawal_processor_lock
      SET locked_until = ?
      WHERE id = 1
        AND locked_until < ?
    ...
    console.error("Withdrawal processor lock error:", error);
    return false;
  }
}

async function releaseWithdrawalProcessorLock(env) {
  try {
    await env.DB.prepare(`
      UPDATE withdrawal_processor_lock
      SET locked_until = 0
...
async function processPendingWithdrawals(
  ...
  const acquired = await acquireWithdrawalProcessorLock(env);

  if (!acquired) {
    return [{
      skipped: true,
      reason: "withdrawal_processor_busy"
    }];
  }
  ...
  try {
    ...
    for (const withdrawal of (rows?.results || [])) {
      try {
        results.push(
          await processWithdrawal(env, withdrawal)
        );
      } catch (error) {
        try {
          await saveWithdrawalError(
        ...
        results.push({
          success: false,
          withdrawal_id: Number(withdrawal.id),
          error_stage: "unexpected",
          error_message: error?.message || "Unexpected withdrawal error",
          refunded: false
        });
      }
    }
    ...
    await releaseWithdrawalProcessorLock(env);
  }
}
  ...
  async fetch(
    ...
    if (
      ...
      return json({
        ...
        toncenter_api_key_configured:
          !!env.TONCENTER_API_KEY,

        fast_withdrawal_trigger:
          true
