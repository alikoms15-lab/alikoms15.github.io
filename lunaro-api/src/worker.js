...
const LNR_MASTER = "EQArZiLoiwGQVFXf-xypL_3qBg6DYuOQTHNJhOmEju-LNPuk";
const TREASURY_OWNER = "EQCNobW-3Kzoku_up0oV0yD3pUegqj0q3S2ptCfp2yhKig0-";
const TONCENTER_URL = "https://toncenter.com/api/v2/jsonRPC";
function sleep(ms){return new Promise(r=>setTimeout(r,ms));}
function round6(n){return Math.round(Number(n)*1000000)/1000000;}
function isValidTonAddress(v){try{Address.parse(String(v));return true;}catch{return false;}}
function rawFromHuman(amount,decimals){const n=Number(amount);if(!Number.isFinite(n)||n<=0)throw new Error("Invalid token amount");return BigInt(Math.round(n*(10**decimals)));}
...
const mnemonic = env.LNR_TREASURY_MNEMONIC;
...
const CONFIG = {
  ...
  MAX_LEVEL: 1000,
  LEVEL_STEP_LNR: 5000,
  BASE_MINING_RATE: 7.5,
  MINING_RATE_STEP: 499 / 999,
  WITHDRAWAL_MIN: 500,
  WITHDRAWAL_FEE: 70,
  WITHDRAWAL_NET: 430,
...
async function settleMining(env, user, wallet = null) {
  ...
  const pool = Math.max(0, num(user.balance));
  const holding = Math.max(0, num(user.holding_wallet));
  const totalAssets = pool + holding;
  const level = Math.min(CONFIG.MAX_LEVEL, Math.max(1, Math.floor(totalAssets / CONFIG.LEVEL_STEP_LNR) + 1));
  const rate = Math.max(0, miningRateForLevel(level));
  ...
  await env.DB.prepare(`
    ...
    WHERE telegram_id = ?
  `).bind(wallet || null, mined, rate, level, nowIso(), String(user.telegram_id)).run();
...
async function holdingWalletBalance(env, wallet) {
  // Direct TON Center v3 indexed lookup:
  // owner wallet + LNR Jetton Master -> user's LNR Jetton Wallet -> balance.
  //
  // IMPORTANT:
  // ok=false means the blockchain/indexer did not give us a valid answer.
  // That is NOT the same thing as a real zero balance.
  if (!safeWallet(wallet)) return { ok: false, balance: 0 };
  ...
  try {
    ...
    // Never turn a temporary API/RPC error into zero.
    if (!response.ok) return { ok: false, balance: 0 };

    const data = await response.json();

    if (!data || !Array.isArray(data.jetton_wallets)) {
      return { ok: false, balance: 0 };
    }

    const item = data.jetton_wallets[0];

    // Because exclude_zero_balance=true is used, no item is a valid
    // blockchain answer meaning the wallet currently has zero LNR.
    if (!item) return { ok: true, balance: 0 };

    // LNR has 6 decimal places.
    ...
    // Keep the last known valid balance when the indexer is unavailable.
    return { ok: false, balance: 0 };
  }
}
...
async function buildUserPayload(env, user) {
  ...
  // Database value is the last known valid Holding Wallet balance.
  let holding = num(user?.holding_wallet, 0);

  const chainResult = await holdingWalletBalance(env, wallet);

  if (chainResult.ok) {
    // A valid blockchain answer is allowed to replace the previous value,
    // including a real zero.
    holding = chainResult.balance;

    // Persist the newest valid value so the next request can use it while
    // waiting for another blockchain response.
    try {
      await env.DB.prepare(
...
async function handleWithdraw(request, env) {
  ...
  try {
    ...
    await env.DB.prepare(`
      ...
      (telegram_id, wallet, amount, fee, net_amount, status)
      VALUES (?, ?, ?, ?, ?, 'pending')
    `).bind(
...
async function handleWithdrawals(request, env) {
  ...
  try {
    const result = await env.DB.prepare(`
      SELECT id, wallet, amount, fee, net_amount, status, tx_hash, created_at, processed_at
      FROM withdrawals
      WHERE telegram_id = ?
      ORDER BY id DESC
      LIMIT 100
...
async function processWithdrawal(env, withdrawal) {
  const id=Number(withdrawal.id);
  const lock=await env.DB.prepare(`UPDATE withdrawals SET status='processing' WHERE id=? AND status='pending'`).bind(id).run();
  if(Number(lock?.meta?.changes||0)!==1)return {skipped:true,reason:"already_processing_or_completed"};
  const payout=Number(withdrawal.net_amount);
  const requestedAmount=Number(withdrawal.amount);
  const wallet=String(withdrawal.wallet||"").trim();
  const telegramId=String(withdrawal.telegram_id||"").trim();
  try{
    if(!wallet)throw new Error("Withdrawal wallet is empty");
    if(!isValidTonAddress(wallet))throw new Error("Invalid withdrawal wallet");
    if(!Number.isFinite(payout)||payout<=0)throw new Error("Invalid payout amount");
    const sendResult=await sendLnr(env,wallet,payout);
    const treasuryInfo=await getTreasuryJettonWallet(env);
    await waitForSeqnoAdvance(treasuryInfo.client,treasuryInfo.treasury.wallet,sendResult.seqno,120000);
    await sleep(3000);
    const txHash=await findRecentTransactionHash(env,treasuryInfo.treasury.wallet.address);
    await env.DB.prepare(`UPDATE withdrawals SET status='completed',tx_hash=?,processed_at=CURRENT_TIMESTAMP WHERE id=? AND status='processing'`).bind(txHash,id).run();
    return {success:true,withdrawal_id:id,telegram_id:telegramId,payout,tx_hash:txHash};
  }catch(error){
    const failed=await env.DB.prepare(`UPDATE withdrawals SET status='failed',processed_at=CURRENT_TIMESTAMP WHERE id=? AND status='processing'`).bind(id).run();
    const changed=Number(failed?.meta?.changes||0);
    if(changed===1&&Number.isFinite(requestedAmount)&&requestedAmount>0){await env.DB.prepare(`UPDATE users SET balance=COALESCE(balance,0)+?,updated_at=CURRENT_TIMESTAMP WHERE telegram_id=?`).bind(requestedAmount,telegramId).run();}
    return {success:false,withdrawal_id:id,error:error?.message||"Transfer failed",refunded:changed===1};
  }
}
async function processPendingWithdrawals(env){
  const rows=await env.DB.prepare(`SELECT id,telegram_id,wallet,amount,fee,net_amount,status,tx_hash,created_at FROM withdrawals WHERE status='pending' ORDER BY id ASC LIMIT 5`).all();
  const results=[];for(const w of (rows?.results||[])){results.push(await processWithdrawal(env,w));await sleep(1000);}return results;
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/process-withdrawals" && request.method === "POST") {
        const secret=request.headers.get("X-LNR-CRON-SECRET");
        if(!env.LNR_CRON_SECRET || secret!==env.LNR_CRON_SECRET)return json({success:false,error:"Unauthorized"},401);
        return json({success:true,results:await processPendingWithdrawals(env)});
      }
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil((async()=>{try{await processPendingWithdrawals(env);}catch(error){console.error("LNR auto-withdraw error:",error);}})());
  }
};
