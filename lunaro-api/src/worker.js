...
async function settleMining(env, user, wallet = null) {
  ...
  // One source of truth for mining speed: the user's current Level.
  // Total Assets = Pool balance + last confirmed Holding Wallet balance.
...
async function handleClaim(request, env) {
  // Single source of truth: settleMining() calculates mining accrual.
  // Claim only moves that settled mined amount into the pool balance.
  const data = await request.json();
  const telegramId = String(data.telegram_id || "");
  const wallet = String(data.wallet || "");
  ...
  // IMPORTANT: use exactly the same mining calculation as /mine.
  const settledUser = await settleMining(env, user, wallet);
  const claimAmount = Math.max(0, num(settledUser.mined, 0));

  if (claimAmount <= 0) {
    return json({ success: false, error: "Nothing to claim" }, 400);
  }

  const newBalance = num(settledUser.balance, 0) + claimAmount;
  ...
  return json({
    success: true,
    claimed: claimAmount,
    user: await buildUserPayload(env, updatedUser)
  });
}
